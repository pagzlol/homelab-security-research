...
The fake AWS credentials are backed by a real IAM user with a deny all policy. If an attacker steals the credentials and tries to use them:
...
This adds a **cloud layer tripwire** on top of the local detection. Even if the attacker steals the credentials before being blocked, the attempt to use them still shows up in AWS.
...
- **Two layer detection** (local auditd + cloud AWS GuardDuty) means the attacker has to evade two independent systems to go undetected
- This pattern is easy to extend. More honeytokens, such as database credentials, API keys, or config files, only need one auditd rule and one Wazuh rule each
