...
On 2026-05-04 a Go SSH scanner made two separate connections to the Cowrie honeypot on fuji, routing through different Tor exit nodes in the same /26 block (torservers.net, AS4224). Both sessions used the same tooling: the same SSH-2.0-Go client, the same HASSH fingerprint, a blank password `root` login, and the same recon payload wrapped in an `echo "..." | sh` pattern designed to avoid shell history logging.
...
This is a scouting pass before the real attack: the operator is sorting targets by runtime environment (bare metal vs Docker vs VM) before deciding whether to send a payload.
...
Both IPs are listed in the public Tor exit node directory. The campaign is fully anonymised at the network layer: the real operator IP is hidden behind the onion routing chain. Attribution beyond "Go scanner using torservers.net exits" is not possible from honeypot data alone.
...
**Blank password root is still a viable credential.** The scanner is not using a dictionary: it targets the blank password case specifically (weak or default configurations, freshly deployed VMs, misconfigured containers). Rate limiting or fail2ban on auth failures won't help against single attempt scanners; key only auth does.
