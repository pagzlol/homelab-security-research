...
This writeup explains the attack surface monitor I built for the homelab. It runs from the Fuji VPS and checks the home server from the outside. It watches for changes in open ports, web fingerprints, DNS records, subdomains, and TLS certificates. When something changes, it sends the event to Wazuh and then to Discord.

The main lesson is simple: **you can't detect what you can't see from the outside.** Scanning from inside your own network shows a different picture from what the internet sees. Running scans from a separate VPS gives me the same view an attacker gets.
...
- **Check from outside**: scanning from inside your own network can lie to you. An outside scanner shows what attackers actually see
