...
Over a five-day window, two source IPs sharing **identical HASSH and SSH client fingerprint** operated as coordinated parallel scan branches against the Cowrie honeypot. Each branch authenticates independently and runs a single-command probe — no downloads, no persistence, immediate disconnect. Neither branch produces a follow-on payload during this window; this is pure pre-exploitation reconnaissance.
...
[No follow-on payload observed in this window]
```
...
| Sessions | 99 |
| Successful logins | 7 |
| Post-auth commands | 1 per successful session |
| Downloads | 0 |
...
| Post-auth probe | `base64` capability check | `/bin/./uname` arch collection |
| Payload | none | none |
...
NINGI-010 (Tor-routed scanner) used HASSH `087ab61de4f8afa9ac8f30c1b7c418eb`. NINGI-011 (mdrfckr spread wave) used `af8223ac9914f509afdadfaf5f7ee94e`. The HASSH here (`16443846184eafde36765c9bab2f4397`) is distinct from both — this is a third, separate Go-based scanning toolchain.
...
| Post-auth probe | bash, `/proc/1/`, cpuinfo, nanosec token | base64 check OR uname arch |
...
| Follow-on payload | None observed this window |
...
The absence of a payload during this observation window means one of three things: (1) the probe is an early-stage scanner that feeds a target queue processed by a separate tool, (2) no collected fingerprint passed the filter for payload delivery during this window, or (3) the follow-on stage operates on a much longer cycle (days or weeks). The base64 check and uname output are not useful in isolation — they become useful when aggregated across thousands of targets to build a delivery-ready list sorted by arch and shell capability.
