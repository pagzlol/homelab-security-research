...
| SSH client | `SSH-2.0-Go` (custom Go scanner) |
...
**The `mdrfckr` key comment is a known Mirai botnet IoC.** This exact public key has appeared in honeypot captures globally and is directly associated with Mirai variants that keep SSH backdoor access on compromised hosts. Any system with this key in `authorized_keys` is owned by the Mirai operator and accessible via the matching private key.
